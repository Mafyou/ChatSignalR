...
public class KITHub(IHubContext<KITHub> hubContext) : Hub<IKITHub>, IKITHub
{
    ...
    {
        await hubContext.Clients.User(settings.Other.Id.ToString()).SendAsync(ChatHubStatus.Initialized, new Initialization());
        
        await hubContext.Clients.User(settings.Me.Id.ToString()).SendAsync(ChatHubStatus.Initialized, new Initialization());
    }
